...
func Open() (*ProjectMemory, error) {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS project_memory (
		...
		db.Close()
		return nil, fmt.Errorf("creating project_memory table: %w", err)
	}
...
func (m *ProjectMemory) ListRecent(limit int) ([]Entry, error) {
	...
	for rows.Next() {
		var e Entry
		var lastSeen string
		if err := rows.Scan(&e.Dir, &e.Branch, &e.LastCommand, &lastSeen, &e.SessionPID, &e.Notes); err != nil {
			return nil, err
		}
		e.LastSeen, _ = time.Parse(time.RFC3339, lastSeen)
