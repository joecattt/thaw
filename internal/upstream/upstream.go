...
func Format(r *Report) string {
	...
	fmt.Fprintf(&b, "  Upstream changes for %s [%s]:\n", r.Dir, r.Branch)

	if r.ForcePushed {
		b.WriteString("  ⚠ upstream was force-pushed — your local may be diverged\n")
	}
	if r.BehindBy > 0 {
		fmt.Fprintf(&b, "  ⚠ %d new commit(s) on upstream — pull needed\n", r.BehindBy)
	...
	if r.PRComments > 0 {
		fmt.Fprintf(&b, "  · %d PR comment(s) to review\n", r.PRComments)
	}
	if len(r.DepFilesChanged) > 0 {
		fmt.Fprintf(&b, "  ⚠ dependency files changed upstream: %s\n", strings.Join(r.DepFilesChanged, ", "))
	}
	return b.String()
}
