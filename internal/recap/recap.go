...
func Generate(store *snapshot.Store, from, to time.Time) (*DayRecap, error) {
	...
	if len(snaps) == 0 {
		// Zero-setup path: no daemon, no snapshots — still deliver value by
		// valuing git history from repos discovered under the working dir.
		roots := DiscoverRepos()
		if effort := EffortForRepos(roots, from, to, DefaultEffortConfig()); effort != nil && len(effort.Commits) > 0 {
			return &DayRecap{
	...
	// Labor-cost valuation from git commits in the repos seen today
	repoSet := make(map[string]bool)
	for _, snap := range snaps {
		for _, s := range snap.Sessions {
			if s.Git != nil && s.Git.RepoRoot != "" {
				repoSet[s.Git.RepoRoot] = true
			}
		}
	}
	var roots []string
	for root := range repoSet {
		roots = append(roots, root)
	}
	recap.Effort = EffortForRepos(roots, from, to, DefaultEffortConfig())
...
func FormatText(r *DayRecap) string {
	...
	if len(r.Transitions) == 0 {
		if r.Effort != nil && len(r.Effort.Commits) > 0 {
			b.WriteString("(no session data — effort valued from git history alone;\n")
			b.WriteString(" run `thaw daemon` to also capture your workspace timeline)\n")
			b.WriteString(FormatEffort(r.Effort))
			return b.String()
		}
	...
	for _, t := range r.Transitions {
		...
		fmt.Fprintf(&b, "%s %s  %s %s", timeStr, icon, t.Action, t.Project)
		if t.Branch != "" {
			fmt.Fprintf(&b, " (%s)", t.Branch)
		}
		b.WriteString("\n")
...
// VoiceConfig holds TTS backend configuration. Voice uses portable system / open-source
// TTS only — no bundled models, no hardcoded paths, no voice cloning. If a backend isn't
// present on the machine, recap stays text-only.
type VoiceConfig struct {
	Backend string // auto | say | piper | espeak
}
...
func SpeakWithConfig(text string, cfg VoiceConfig) error {
	...
	switch backend {
	case "piper":
		// Piper (MIT, local neural TTS) → raw PCM → platform playback.
		cmd := exec.Command("piper", "--model", "en_US-lessac-medium", "--output-raw")
		cmd.Stdin = strings.NewReader(text)
		var player *exec.Cmd
		if runtime.GOOS == "darwin" {
			// afplay can't read raw stdin; piper isn't a common macOS backend, fall back to say.
			return exec.Command("say", "-r", "180", text).Run()
		} else if _, err := exec.LookPath("paplay"); err == nil {
			player = exec.Command("paplay", "--raw", "--rate=22050", "--format=s16le", "--channels=1")
		} else {
			player = exec.Command("aplay", "-r", "22050", "-f", "S16_LE", "-t", "raw", "-")
		}
		player.Stdin, _ = cmd.StdoutPipe()
	...
	default: // "say" or fallback
		...
		return fmt.Errorf("no TTS backend available — install piper or espeak, or set tts_backend in config")
	}
}
...
// detectBestBackend picks a portable TTS backend that's actually installed.
func detectBestBackend() string {
	if runtime.GOOS == "darwin" {
		return "say" // built in on macOS
	}
...
.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:48px}
...
<div class="version">thaw v`+buildinfo.Version+`</div>
</div>
</body></html>`,
		r.Date,
		...
		r.Date,
		r.LastSeen.Format("3:04 PM"),
		buildMetricsHTML(len(r.Projects), len(r.Transitions), r.TotalTime, r.Effort),
		timelineRows.String(),
		projectRows.String(),
...
func buildParticlesHTML(n int) string {
	...
	for i := 0; i < n; i++ {
		...
		fmt.Fprintf(&b, `<div class="p" style="left:%.0f%%;top:%.0f%%;width:%.1fpx;height:%.1fpx;opacity:%.3f;--dx:%.0fpx;--dy:%.0fpx;animation:drift %.0fs ease-in-out %.0fs infinite alternate"></div>`,
...
func buildCrystalArmsHTML() string {
	...
	for i, r := range []int{0, 60, 120, 180, 240, 300} {
		...
		fmt.Fprintf(&b, `<g transform="rotate(%d)"><rect fill="%s" x="-2.5" y="%d" width="5" height="%d" rx="1.5"/></g>`,
			r, fill, y, h)
	}
	return b.String()
}

func buildMetricsHTML(projects, sessions int, total time.Duration, effort *EffortReport) string {
	...
	if effort != nil && len(effort.Commits) > 0 {
		metrics = append(metrics, struct{ label, value string }{
			"labor value", fmt.Sprintf("$%s–$%s",
				formatDollars(effort.TotalCostLow), formatDollars(effort.TotalCostHigh)),
		})
	}
	var b strings.Builder
	for i, m := range metrics {
		fmt.Fprintf(&b, `<div class="glass metric" style="animation-delay:%.2fs"><div class="metric-label">%s</div><div class="metric-val">%s</div></div>`,
