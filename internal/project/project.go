...
// Info summarizes one project's worth of sessions from a snapshot, grouped
// by GroupName (or the CWD basename when no group is set).
type Info struct {
	Name     string
	Branch   string
...
// Group buckets snapshot sessions by project and sorts the result by
// descending session count. This is the grouping the interactive restore
// picker displays ("Projects to restore: 1) ... 2) ...").
func Group(sessions []models.Session) []*Info {
	...
	sorted := make([]*Info, 0, len(order))
	for _, k := range order {
		sorted = append(sorted, projects[k])
	}
	for i := 0; i < len(sorted); i++ {
		for j := i + 1; j < len(sorted); j++ {
			if len(sorted[j].Sessions) > len(sorted[i].Sessions) {
