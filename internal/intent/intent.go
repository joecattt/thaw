...
func buildPrompt(snap *models.Snapshot) string {
	...
	for i, s := range snap.Sessions {
		...
		fmt.Fprintf(&b, "%d. CWD=%s CMD=%s", i+1, cwd, cmd)
		if s.Git != nil {
			fmt.Fprintf(&b, " BRANCH=%s", s.Git.Branch)
			if s.Git.Dirty {
				b.WriteString(" DIRTY")
			}
		}
		if len(s.History) > 0 {
			...
			fmt.Fprintf(&b, " RECENT=[%s]", strings.Join(redacted, "; "))
		}
		b.WriteString("\n")
	}
...
// /home/alice/projects/secret-client/api → .../secret-client/api
