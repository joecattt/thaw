...
func rcFilePath(shell string) string {
	...
	default:
		// fish (and anything else) falls through: `thaw shell-init` only
		// emits zsh/bash hooks, so injecting an eval line would break the
		// shell on every start. The caller prints the add-manually hint.
		return ""
	}
}
