...
No C compiler needed — SQLite is pure Go via `modernc.org/sqlite`, and all
builds (Makefile, CI, releases) run with `CGO_ENABLED=0`.
...
```
cmd/thaw/             CLI commands (Cobra) — one cmd_*.go file per command,
                      wired together in main.go
...
1. Create `cmd/thaw/cmd_yourcommand.go` with a `yourCommandCmd() *cobra.Command`
   function (match the pattern in any existing `cmd_*.go` file)
2. Register it in `main()` (in `cmd/thaw/main.go`) under `root.AddCommand()`
   or `admin.AddCommand()`
...
Tags trigger a goreleaser build. To release:
...
GitHub Actions builds binaries for macOS (arm64/amd64) and Linux (arm64/amd64)
and pushes the Homebrew formula.
